...
| C13 | **双端感知与共定位**（PS 轨：`ObservationLabel` 检测结果 / `scene_graph` 广播 / 共定位状态与 `colocalization_stale`） | v1.1 | canonical（[`contracts/PERCEPTION_COLOCALIZATION_CONTRACT.md`](contracts/PERCEPTION_COLOCALIZATION_CONTRACT.md)） | Daedalus（机器人侧感知+融合）·Eidolon（头显侧消费+标注上报） | 待建（schema + conformance 见 [canonical#77](https://github.com/Mnesis-Labs/mnesis-canonical/issues/77) 收尾项） |
