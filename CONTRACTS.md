...
- **2026-08-11 · 新 profile `ego_multicam_v1`（issue #115，Argus DatCap 多相机 ego 设备）**：新增 profile，开放相机键集 `observation.images.<camera_name>`，相机名取自 embodiment registry `capture.cameras[].name`（不是自由字符串，校验时比对 registry 拼错即报错）。`ego_v1` / `robot_v2` 零改动，现有数据零迁移。`observation.state` / `action` 保持定长（`float[7]` / `float[6]`），`embodiment_id` 必填（相机名解析依赖 registry），缺帧相机 = 键缺失（铁律「缺失 = 未知，禁带内哨兵」）。多帧率 rig 转换器侧决定参考相机，非新 wire 字段。SPEC.md / `canonical_frame.schema.json` / `mnesis_canonical.{schema,validate}` / conformance tests 同步。
