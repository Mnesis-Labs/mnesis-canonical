...
## 契约 vNext 提案（来自 Eidolon/Tech-Lead 2026-07-10 · 建议，非实现）

> 来源：https://github.com/Mnesis-Labs/Mnesis-Eidolon/blob/feature/mvp-hand-and-scene/docs/CROSS_REPO_SUGGESTIONS_2026-07-10.md 。先登记不实现，等 Muso 拍板再进 SPEC。与已列 C1-vNext/C6/C7 不重叠。

- **C8 · `action` 缺夹爪/末端执行通道**（致命）：现 `action=float[6]` 无夹爪维，`observation.state[7]` 是 head_pose 镜像非关节+夹爪 → "抓/放"在 canonical 不可表示。两面现锁 1.0，真机操作训练一开跑即硬阻塞。建议加可选 `action.gripper`(0开1闭)，teleop/robot 共用。**任何 manipulation 训练前必须。**
- **C9 · `camera_intrinsics`/`camera_extrinsics` 字段**：两采集面都在采内参（Iris getImageIntrinsics / Eidolon PCA），SPEC 无字段 → 各塞私有键，4DGS/像素→世界重投影拿不到统一来源。
- **C10 · Isaac 三 open item 定"冻结截止日"**：四元数序(xyzw vs Isaac wxyz)/世界系 up-axis 手性/action 旋转表示。Eidolon 是 Unity 左手系，发帧前必须知目标系。建议裁决"wire 保持 xyzw / Y-up，Isaac 走 adapter" + 决策日期。
- **C11 · `hand_skeleton`/body-pose 可选字段决策**（xMimic 门槛 2026-07-31）：Eidolon 现只采 head_pose+action 无骨架；xMimic 要骨架级 teleop 数据。该 canonical 定义而非各面私造。**决策优先于实现。**
