...
| C12 | **WebRTC 信令**（视频流协商：`video_offer`/`video_answer`/`video_ice`；`stream_id`+`subscriber_id` 支持同一路源多订阅者各自协商） | v1.1 | canonical（[`contracts/WEBRTC_SIGNALING_CONTRACT.md`](contracts/WEBRTC_SIGNALING_CONTRACT.md)） | Daedalus（传输层）·Eidolon（消费层） | 待建（schema + conformance 见 [canonical#60](https://github.com/Mnesis-Labs/mnesis-canonical/issues/60) 收尾项） |
