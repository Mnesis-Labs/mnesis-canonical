"""Conformance tests for the C8 gripper channel (D-18, additive).

The gripper is a continuous scalar in [0, 1] carried as an optional first-class
field: ``observation.gripper`` (single/main) and ``observation.gripper.{left,right}``
(bimanual robot_v2). Semantics mirror the C3 xr_bridge wire field ``arms[].gripper``.
All keys are optional and additive — frames without them validate unchanged.
"""
